	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw @${num.split('@')[0]}foi tarde🙄🤙`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '❌ Este comando só pode ser usado pelo proprietário de grupo! ❌',
			...
			const groupebers = isGroup ? groupMetadata.participants : ''
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha, no momento da conversão ${tipe} para a figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
               ...
               case 'hentai1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://hentaiseason.com/wp-content/uploads/2021/01/pack_imagem_kimetsu_no_yaiba_05.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hentaizin😳👉👈'})
					break
               case 'hentai2':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://hentaiseason.com/wp-content/uploads/2021/01/pack_imagem_kimetsu_no_yaiba_11.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hentaizin😳👉👈'})
					break              
		       case 'cos1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://figure.superhentais.com/img/figure/108476.jpeg `)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Carai mikasa 😳'})
					break
		       case 'cos2':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://figure.superhentais.com/img/figure/108309.jpeg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Batata ?😳'})
					break
		       case 'urlx':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://hentaiseason.com/wp-content/uploads/2020/11/pack_imagens_naruto_38.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '⚜️2 Melhores site de hentai⚜️\nhttps://hentaiseason.com/category/pack-de-imagens/ \nhttps://www.hentaistube.com/ \n3 ✨Melhores de porno⚜️\nhttps://www.xvideos.com/ \nhttps://pt.pornhub.com/ \nhttps://www.redtube.com.br/'})
					break
               case 'id':
		            if (args.length < 1) return reply ('⚜️cy/vi/tr/th/ta/sv/es-us/es-es/es/sk/sr/ru/ro/pt-br/pt/pl/no/mk/lv/la/ko/ja/it/id/is/hu/hi/ht/el/de/fr/fi/eo/en-us/en-uk/en-au/en/nl/da/cs/hr/zh-yue/zh-tw/zh/ca/hy/ar/sq/af⚜️')
		            break
				case 'setprefix':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: ':3 Aq seu(sua) lindo (a)>//<'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
